class Carrier::Azul < Carrier
  class << self
    ...
    def get_awb(shipment)
      ...
      response = request.get("api/Ocorrencias/Consultar?token=#{token}&ChaveNFE=#{nfe_key}")
    ...
    def get_delivery_updates(shipment)
      ...
      response = request.get("api/Ocorrencias/Consultar?Token=#{token}&AWB=#{shipment.tracking_number}")
      check_response(response)
    ...
    def authenticate_user(account, user, password, cpf_cnpj)
      ...
      response    = request.post("api/Autenticacao/ValidarUsuarioPortalClienteEdi", credentials)
      if response.body["HasErrors"]
    ...
    def send_to_azul(account, encoded_xml)
      ...
      response = request.post("api/NFe/Enviar?token=#{token}",body)
    end

    def request
      Faraday.new(url: "http://ediapi.onlineapp.com.br") do |conn|
        conn.request :json
